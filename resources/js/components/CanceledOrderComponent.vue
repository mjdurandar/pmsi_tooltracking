            ...
            <template #modalBody>
                <div class="row">
                    ...
                    <div class="col-6">
                        ...
                        <p>
                            You canceled it At : <b>{{this.dataValues.canceled_order_at}}</b>
                        </p>
                        <p>
...
export default{
    ...
    data(){
        return{
                ...
                selectedBrand: '',
                selectedTool: '',
                columns : ['brand_name', 'tool_name', 'placed_order_at', 'canceled_order_at' ,'action'],
                errors: [],
                options : {
                    headings : {
                        brand_name : 'Brand',
                        tool_name : 'Tool',
                        placed_order_at : 'Placed Order At',
                        canceled_order_at : 'Canceled Order At',
                        action : 'Action',
                    },
                    filterable: false,
                    sortable: []
    ...
    methods: {
        ...
        getData() {
            axios.get('/canceled-order/show').then(response => {
        ...
        filterData() {
            ...
            axios.post('/canceled-order/filterData', searchData)
                .then(response => {
                    this.data = response.data.data;
                    this.data.forEach(item => {
                        item.created_at = new Date(item.created_at).toLocaleString(); // Format to the user's locale
